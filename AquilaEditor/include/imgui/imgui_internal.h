...
enum ImGuiNavRenderCursorFlags_
{
    ...
    ImGuiNavRenderCursorFlags_AlwaysDraw    = 1 << 2,       // Render rectangular highlight if (g.NavId == id) even when g.NavCursorVisible == false, aka even when using the mouse.
...
//#endif
    ...
    IMGUI_API bool          RenderTextEllipsis(ImDrawList* draw_list, const ImVec2& pos_min, const ImVec2& pos_max, float ellipsis_max_x, const char* text, const char* text_end, const ImVec2* text_size_if_known);
