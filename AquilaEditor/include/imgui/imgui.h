...
enum ImGuiTreeNodeFlags_
{
    ...
    // [EXPERIMENTAL] Render lines connecting TreeNode hierarchy. Discuss in GitHub issue #2920.
...
enum ImGuiTabBarFlags_
{
    ...
    ImGuiTabBarFlags_DrawSelectedOverline           = 1 << 6,   // Render selected overline markers over selected tab
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_ColorMarkers       = 1 << 12,      // DragScalarN(), SliderScalarN(): Render R/G/B/A color markers on each component.
...
struct ImFontBaked
{
    ...
    unsigned int                LoadNoRenderOnLayout:1;// 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX *without* rendering/packing glyphs. Only advantageous if you know that the glyph is unlikely to actually be rendered, otherwise it is slower because we'd do one query on the first CalcTextSize and one query on the first Render.
