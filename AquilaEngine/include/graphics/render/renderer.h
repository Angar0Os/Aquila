...
namespace graphics::render
{
	...
	struct PassAttachment
	{
		...
		graph::ResourceState                state = graph::ResourceState::Undefined;
	};
	...
	private:
		...
		void BuildAndExecuteFrameGraph(core::gpu::CommandBuffer& _cmdBuf, core::gpu::Image& _outputImage);
		...
		graph::ResourceAllocator			m_resourceAllocator{ m_device };
		graph::RenderGraph					m_renderGraph{ &m_resourceAllocator };

		std::vector<std::unique_ptr<graph::Pass>> m_framePasses;
		std::deque<graph::Resource> m_frameResourcePool;
