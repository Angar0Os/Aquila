...
void CommandBuffer::TraceRays(const Device& _device, 
	...
	uint32_t _missCount, void* _hitSBT, uint32_t _hitOffset, 
	uint32_t _hitStride, uint32_t _hitCount, void* _callableSBT, 
	uint32_t _callableOffset, uint32_t _callableStride, uint32_t _callableCount, 
	uint32_t _width, uint32_t _height, uint32_t _depth)
{
	vk::StridedDeviceAddressRegionKHR raygenRegion{};
	if (_raygenSBT)
	{
		vk::Buffer raygenBuffer = static_cast<VkBuffer>(_raygenSBT);
		vk::BufferDeviceAddressInfo addressInfo{};
		addressInfo.buffer = raygenBuffer;

		raygenRegion.deviceAddress = _device.GetImpl().device.getBufferAddress(addressInfo) + _raygenOffset;
		raygenRegion.stride = _raygenStride;
		raygenRegion.size = _raygenStride;
	}

	vk::StridedDeviceAddressRegionKHR missRegion{};
	if (_missSBT && _missCount > 0)
	{
		vk::Buffer missBuffer = static_cast<VkBuffer>(_missSBT);
		vk::BufferDeviceAddressInfo addressInfo{};
		addressInfo.buffer = missBuffer;

		missRegion.deviceAddress = _device.GetImpl().device.getBufferAddress(addressInfo) + _missOffset;
		missRegion.stride = _missStride;
		missRegion.size = _missStride * _missCount;
	}

	vk::StridedDeviceAddressRegionKHR hitRegion{};
	if (_hitSBT && _hitCount > 0)
	{
		vk::Buffer hitBuffer = static_cast<VkBuffer>(_hitSBT);
		vk::BufferDeviceAddressInfo addressInfo{};
		addressInfo.buffer = hitBuffer;

		hitRegion.deviceAddress = _device.GetImpl().device.getBufferAddress(addressInfo) + _hitOffset;
		hitRegion.stride = _hitStride;
		hitRegion.size = _hitStride * _hitCount;
	}

	vk::StridedDeviceAddressRegionKHR callableRegion{};
	if (_callableSBT && _callableCount > 0)
	{
		vk::Buffer callableBuffer = static_cast<VkBuffer>(_callableSBT);
		vk::BufferDeviceAddressInfo addressInfo{};
		addressInfo.buffer = callableBuffer;

		callableRegion.deviceAddress = _device.GetImpl().device.getBufferAddress(addressInfo) + _callableOffset;
		callableRegion.stride = _callableStride;
		callableRegion.size = _callableStride * _callableCount;
	}

	m_impl->commandBuffer.traceRaysKHR(
		raygenRegion,
		missRegion,
		hitRegion,
		callableRegion,
