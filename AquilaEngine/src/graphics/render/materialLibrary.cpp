...
{
	AQUILA_CHECK(_handle != INVALID_MATERIAL && _handle < materials.size(), "MaterialLibrary: invalid MaterialHandle");
...
void MaterialLibrary::UploadGPUData()
{
	...
	if (!gpuBuffer)
	{
		...
		AQUILA_SET_DEBUG_NAME(device, *gpuBuffer, "MaterialLibrary::GPUBuffer");
	}
	...
	{
		data.push_back(BuildGPUData(handle));
	}
...
{
	AQUILA_CHECK(gpuBuffer != nullptr, "MaterialLibrary: material GPU buffer has not been created");
	return *gpuBuffer;
}
