	...
	core::gpu::utils::ETextureFormat _format)
{
	...
	cmdBuf->Record([&]() {
		cmdBuf->TransitionImageLayout(*image, core::gpu::utils::EImageLayout::TransferDst, false);
		cmdBuf->CopyBufferToImage(*staging, *image, _width, _height);
		cmdBuf->TransitionImageLayout(*image, core::gpu::utils::EImageLayout::ShaderReadOnly, false);
		});
...
{
    if (Contains(_name)) return Find(_name);
	
	AQUILA_CHECK(!_name.empty(), "TextureLibrary: texture name cannot be empty");
	AQUILA_CHECK(_image != nullptr, "TextureLibrary: image is null");
	AQUILA_CHECK(_texture != nullptr, "TextureLibrary: texture is null");

    TextureHandle handle = static_cast<TextureHandle>(textures.size());
	AQUILA_CHECK(handle != INVALID_TEXTURE, "TextureLibrary: texture handle overflow");
	...
	AQUILA_SET_DEBUG_NAME(device, *textures.back().image, _name + "::Image");
	AQUILA_SET_DEBUG_NAME(device, *textures.back().texture, _name + "::Texture");
...
const core::gpu::Texture& TextureLibrary::Get(TextureHandle _handle) const
{
	...
	/*AQUILA_CHECK(_handle != INVALID_TEXTURE && _handle < textures.size(), "TextureLibrary: invalid texture handle");
	AQUILA_CHECK(textures[_handle].texture != nullptr, "TextureLibrary: texture resource is null");
