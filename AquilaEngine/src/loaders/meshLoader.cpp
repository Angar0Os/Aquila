    ...
    int _materialIndex)
{
    ...
    std::cout << pbr.metallicFactor << std::endl;
    material.roughness.value = static_cast<float>(pbr.roughnessFactor);
    std::cout << pbr.roughnessFactor << std::endl;
    ...
    if (pbr.metallicRoughnessTexture.index >= 0)
    {
        ...
        std::cout << "  [ORM] gltf texture index=" << pbr.metallicRoughnessTexture.index
            << " -> textureHandle=" << material.ormTexture
            << (material.ormTexture == INVALID_TEXTURE ? " (INVALID/FALLBACK)" : " (OK)")
            << "\n";
    }
    else
    {
        std::cout << "  [ORM] no metallicRoughnessTexture on this material (index="
            << pbr.metallicRoughnessTexture.index << ")\n";
    }
